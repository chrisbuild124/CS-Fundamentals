# @chrisbuild124
# This is Insertion Sort and uses matplotlib to visualize it.
...
def main():
    ...
    """
    bar_number, time_interval = input_insertion_sort()
    random_list = generate_random_list(bar_number)
    insertion_sort(random_list, time_interval)


# Input the bar number. Must be an integer from 1-50 inclusive.
def input_insertion_sort():
    """
    Description: This is the main function for the program.
...
# Insertion Sort Loop
def insertion_sort(height_list, time_interval):
    """
    Description:          Insertion sort and graphs the output.
    ...
    :returns:             None
    """
    # -1 is the start and j is each next iteration. Ignore the graph
    # lines since that could be confusing.
    first_bar = -1
    for i in range(len(height_list) - 1):
        graph(first_bar, height_list, time_interval)
        if first_bar == -1:
            graph(2, height_list, time_interval)
...
def graph(j, height_list, time_interval):
    ...
    plt.title('Insertion Sort Visualization by @chrisbuild124')
    plt.xticks([])
    plt.yticks([])
